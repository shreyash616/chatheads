...
const mapDispatchToProps = (dispatch) => ({
  searchChatheads: (searchDetails) =>
    dispatch(actions.chatsActions.searchChatheads(searchDetails)),
  sendMessage: (messageDetails) =>
    dispatch(actions.chatsActions.sendMessage(messageDetails)),
  updateUserId: (userId) => dispatch(actions.chatsActions.updateUserId(userId)),
  initiateSignIn: (signInDetails) =>
    dispatch(actions.signInActions.initiateSignIn(signInDetails)),
  clearUpdateUserIdData: () =>
    dispatch(actions.chatsActions.clearUpdateUserIdData()),
  clearSendingMessageData: () =>
    dispatch(actions.chatsActions.clearSendingMessageData()),
  getMessages: (userDetails) =>
    dispatch(actions.chatsActions.getMessages(userDetails)),
  markRead: (userDetails) =>
    dispatch(actions.chatsActions.markRead(userDetails)),
  clearMarkReadData: () => dispatch(actions.chatsActions.clearMarkReadData()),
  clearGetMessagesData: () =>
    dispatch(actions.chatsActions.clearGetMessagesData()),
});
...
const Chats = (props) => {
  ...
  useEffect(() => {
    if (props.signInData.status === "success") {
      const userDetailsFromApi = getResponseKey(
        ["data", "data", "userData"],
        props.signInData
      );
  ...
  useEffect(() => {
    ...
    channel2.bind("mark-read", function (data) {
      const userIdsModified = getResponseKey(
        ["message", "userIdsModified"],
        data
      );
      if (userIdsModified && userIdsModified.includes(userDetails.userId)) {
        props.getMessages({
          jwtToken: props.homeData.data
            ? props.homeData.data.data.jwtToken
            : null,
          data: {
  ...
  useEffect(() => {
    ...
    const unreadFlag =
      ...
      selectedChat.messages.find((msg) => msg.unread === true);
    if (!!unreadFlag) {
      const payload = {
        jwtToken: props.homeData.data
          ? props.homeData.data.data.jwtToken
  ...
  useEffect(() => {
    ...
    if (messagesStatus === "success") {
      const chats = getResponseKey(
        ["data", "data", "userData", "chats"],
  ...
  useEffect(() => {
    ...
    if (searchText) {
      ...
      debounce(() => props.searchChatheads(searchDetails), 1000)();
    }
  }, [searchText]);
  ...
  const handleClickOnChatAfterSearch = (chat) => {
    const usersChat =
      ...
      userDetails.chats.filter(
        (userChat) => userChat.userId === chat.userId
      )[0];
    ...
    if (message) {
      ...
      setSelectedChat({
        ...
        messages: selectedChat.messages
          ? [
              ...selectedChat.messages,
              { message, userId: userDetails.userId, read: "waiting" },
            ]
          : [{ message, userId: userDetails.userId, read: "waiting" }],
      });
    }
      ...
      <IconWrapper>
        ...
        >
          <svg
            ...
            className={
              props.theme === "dark"
                ? "svg-icon-orange icon-margin-top"
                : "svg-icon-blue icon-margin-top"
            }
            viewBox="0 0 20 20"
          >
  ...
  const getChats = () => {
    if (selectedChat.messages) {
      const firstUnreadIndex = selectedChat.messages.findIndex(
        (msg) => msg.unread === true
      );
        ...
        if (messageObj.userId === userDetails.userId) {
          return (
            <React.Fragment key={i}>
              <SentMessage theme={props.theme}>
                {messageObj.message}
                {messageObj.read === "waiting" ? (
                  <WaitIcon
          ...
          return (
            <React.Fragment key={i}>
              {i === firstUnreadIndex && (
                <UnreadMarker theme={props.theme}>Unread</UnreadMarker>
              )}
              <ReceivedMessage theme={props.theme}>
                {messageObj.message}
              </ReceivedMessage>
              <ReceivedTrDown
                tabIndex={-1}
              ...
              return (
                <Chathead>
                  <ChatheadsCircles
                    ...
                    {...props}
                  >
                    {chat.messages.find((msg) => msg.unread === true) && (
                      <ChatheadBadge theme={props.theme} />
                    )}
                  </ChatheadsCircles>
  ...
  const Profile = () => (
    ...
    >
      ...
      <br aria-hidden />
      <UsernameWrapper>
        <UsernameLabel theme={props.theme}>
          {chatConstants.NAME_LABEL}
        </UsernameLabel>
        <Username theme={props.theme}>{userDetails.name}</Username>
      </UsernameWrapper>
      <UsernameWrapper>
        <UsernameLabel shiftDown={showUpdateField} theme={props.theme}>
          {chatConstants.USERNAME_LABEL}
        </UsernameLabel>
        <UsernameWrapper persistent>
          ...
          {!showUpdateField && (
            <UpdateButtonWrapper>
              <svg
                role="button"
                onClick={triggerUpdateField}
                height="1em"
                width="1em"
                className={
                  props.theme === "dark" ? "svg-icon-orange" : "svg-icon-blue"
                }
                viewBox="0 0 20 20"
              >
                <path
    ...
    <ChatWrapper {...props}>
      {selectedChat ? (
        <React.Fragment>
          ...
          <ChatWindow>{getChats()}</ChatWindow>
        ...
        <NoChatSelectedMessage theme={props.theme}>
          {chatConstants.NO_CHAT_SELECTED_MESSAGE}
        </NoChatSelectedMessage>
      )}
    </ChatWrapper>
  );
      ...
      {!searchText ? (
        <ChatheadWrapper theme={props.theme}>
          ...
          {userDetails && userDetails.chats.length === 0 && (
            <NoChatheadsMessage theme={props.theme}>
              {chatConstants.NO_CHATS_MESSAGE}
            </NoChatheadsMessage>
          )}
        </ChatheadWrapper>
      ) : (
  ...
  const ProfileChatsOptions = () => (
    <DetailsWrapper theme={props.theme}>
      <ProfileTitle
        onClick={() => setShowChats(false)}
        isClicked={!showChats}
        theme={props.theme}
      >
        {chatConstants.PROFILE_TITLE}
      </ProfileTitle>
      <ChatsTitle
